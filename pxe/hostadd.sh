...
break
    ...
    #------------------------------------------------
    # Restorelastmodif - restauration du serveur avant la derniere modif
    #------------------------------------------------
    Restorelastmodif()
        {

		echo -e "\n\tcette procedure va remettre la configuration a l'etat n-1."
		echo -e "\n\tetes-vous sur de vouloir continuer? [y/n]"
		read confirm

		if [ "$confirm" != "y" ]; then
		   echo "prudence est mere de surete"
		   exit 1
		fi

		echo -e "\tremise du fichier /etc/dhcp/dhcpd.conf n -1"

		cp -p /opt/scripts/pxe/backup/dhcpd.conf $DHCPFILE

		/etc/init.d/isc-dhcp-server restart

		sleep 5

		echo -e "\tsuppression du dernier fichier d'adresse mac cree dans $PXEROOTDIR et $PXEREALNAMEDIR"

		lastpxerealname=`ls -t $PXEREALNAMEDIR | grep -v realnames | head -1`

		lastpxebootfile=`ls -t $PXEROOTDIR | grep -v realnames | head -1`

		unlink $PXEREALNAMEDIR/$lastpxerealname
		rm -f $PXEROOTDIR/$lastpxebootfile
   ...
   # M A I N . . .
    ...
    Menu \
      "############################### Menu PXE #################################################" \
       ...
       Restorelastmodif "Remise du serveur a l'etat n -1"
